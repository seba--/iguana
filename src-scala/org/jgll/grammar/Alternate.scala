...
trait AlternateTrait {
  ...
  class Alternate(@BeanProperty val firstSlot: BodyGrammarSlot) extends Serializable {
    ...
    def getSymbolAt(index: Int): Symbol = symbols(index).getSymbol
    ...
    def getSlotAt(index: Int): BodyGrammarSlot = symbols(index)

    def getLastSlot(): BodyGrammarSlot = symbols(symbols.size - 1)

    def size(): Int = symbols.size

    def getNonterminalAt(index: Int): HeadGrammarSlot = {
      val bodyGrammarSlot = symbols(index)
    ...
    def isBinary(head: HeadGrammarSlot): Boolean = {
      if (!(symbols(0).isInstanceOf[NonterminalGrammarSlot] &&
        symbols(symbols.size - 1).isInstanceOf[NonterminalGrammarSlot])) {
        return false
      }
      val firstNonterminal = symbols(0).asInstanceOf[NonterminalGrammarSlot]
      val lastNonterminal = symbols(symbols.size - 1).asInstanceOf[NonterminalGrammarSlot]
    ...
    def isUnaryPrefix(head: HeadGrammarSlot): Boolean = {
      ...
      if (!(symbols(index).isInstanceOf[NonterminalGrammarSlot])) {
        return false
      }
      val firstNonterminal = symbols(index).asInstanceOf[NonterminalGrammarSlot]
      head.getNonterminal == firstNonterminal.getNonterminal.getNonterminal
    }
    ...
    def isUnaryPostfix(head: HeadGrammarSlot): Boolean = {
      ...
      val lastNonterminal = symbols(index).asInstanceOf[NonterminalGrammarSlot]
      head.getNonterminal == lastNonterminal.getNonterminal.getNonterminal
    }
    ...
    def `match`(list: ListBuffer[Symbol]): Boolean = {
      ...
      for (i <- 0 until symbols.size if symbols(i).getSymbol != list(i)) {
        return false
      }
      true
    }
