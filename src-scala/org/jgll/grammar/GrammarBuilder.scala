...
import org.jgll.grammar.condition._
import org.jgll.grammar.grammaraction.LongestTerminalChainActionTrait
...
import org.jgll.grammar.slotaction._
...
trait GrammarBuilderTrait {
    self: BodyGrammarSlotTrait
     ...
     with NotPrecedeActionsTrait
     with NotFollowActionsTrait
     with KeywordConditionTrait
     with NotMatchActionsTrait
     with TerminalConditionTrait
     with TerminalTrait
     with LongestTerminalChainActionTrait
     with EpsilonTrait
     with KeywordGrammarSlotTrait
     with CharacterTrait
     with EOFTrait
     with DirectNullableNonterminalGrammarSlotTrait
     with LineActionsTrait
  =>
  ...
  class GrammarBuilder(var name: String) extends Serializable {
    ...
    var newNonterminalsMap: Map[Nonterminal, ListBuffer[HeadGrammarSlot]] = HashMap()

    private var existingAlternates: Map[Set[Alternate], HeadGrammarSlot] = HashMap()

    private var precednecePatternsMap: Map[Nonterminal, ListBuffer[PrecedencePattern]] = HashMap()

    private var exceptPatterns: ListBuffer[ExceptPattern] = ListBuffer()

    private var ruleToLastSlotMap: Map[Rule, LastGrammarSlot] = HashMap()

    private var reachabilityGraph: Map[HeadGrammarSlot, Set[HeadGrammarSlot]] = HashMap()
    ...
    def addRules(rules: Iterable[Rule]): GrammarBuilder = {
      for (rule <- rules) {
        addRule(rule)
      }
      this
    ...
    def addRule(rule: Rule): GrammarBuilder = {
      ...
      val conditions = HashMap[BodyGrammarSlot, ListBuffer[Condition]]()
      val head = rule.head
    ...
    private def addCondition(slot: BodyGrammarSlot, condition: Condition) = condition.getType match {
      ...
      case NOT_FOLLOW => if (condition.isInstanceOf[TerminalCondition]) {
        NotFollowActions.fromTerminalList(slot.next(), condition.asInstanceOf[TerminalCondition].terminals)
      ...
      case NOT_PRECEDE =>
        ...
        if (condition.isInstanceOf[KeywordCondition]) {
          ...
          NotPrecedeActions.fromTerminalList(slot, terminalCondition.terminals)
        }
    ...
    private def calculateExpectedDescriptors() {
      ...
      for (head <- nonterminals) {
        ...
        indirectReachableNonterminals --= (directReachableNonterminals)
        for (nt <- indirectReachableNonterminals) {
          num += nt.getCountAlternates
        }
        expectedDescriptors += (num)
      }
    ...
    def addAll[T](s: Set[T], s2: Set[T]): Boolean = {
      var changed = false
      for (el <- s2)
        changed |= s.add(el)
      changed
    ...
    private def addFirstSet(set: Set[Terminal], currentSlot: BodyGrammarSlot, _changed: Boolean): Boolean = {
      ...
      if (currentSlot.isInstanceOf[EpsilonGrammarSlot]) {
        ...
        set.add(currentSlot.asInstanceOf[KeywordGrammarSlot].getKeyword.getFirstTerminal) ||
          changed
        ...
        changed = addAll(set, nonterminalGrammarSlot.getNonterminal.getFirstSet) || changed
            ...
            while (!(currentSlot.isInstanceOf[LastGrammarSlot])) {
              if (currentSlot.isInstanceOf[NonterminalGrammarSlot]) {
                ...
                if (next.isInstanceOf[LastGrammarSlot]) {
                  changed |= addAll(nonterminalGrammarSlot.getNonterminal.getFollowSet, head.getFollowSet)
                  return
                }
    ...
    private def doGroupPatterns[T <: AbstractPattern](patterns: ListBuffer[T]): Map[T, Set[ListBuffer[Symbol]]] = {
      val group = HashMap[T, Set[ListBuffer[Symbol]]]()
    ...
    private def doCopyAlternates(head: HeadGrammarSlot, list: ListBuffer[Alternate]): ListBuffer[Alternate] = {
    ...
    private def doCopyAlternates(head: HeadGrammarSlot, list: Set[Alternate]): ListBuffer[Alternate] = {
      val copyList = ListBuffer[Alternate]()
      for (alt <- list) {
        copyList += (copyAlternate(alt, head))
      }
    ...
    def addPrecedencePattern(nonterminal: Nonterminal,
      ...
      if (precednecePatternsMap.isDefinedAt(nonterminal)) {
        precednecePatternsMap.get(nonterminal).get += (pattern)
      } else {
    ...
    private def calculateReachabilityGraph() {
      ...
      while (changed) {
        ...
        for (head <- allNonterminals) {
          var set = reachabilityGraph.getOrElse(head, Set[HeadGrammarSlot]())
    ...
    private def calculateReachabilityGraph(set: Set[HeadGrammarSlot], currentSlot: BodyGrammarSlot, _changed: Boolean): Boolean = {
      ...
      if (currentSlot.isInstanceOf[EpsilonGrammarSlot]) {
        ...
        var set2 = reachabilityGraph.getOrElse(nonterminalGrammarSlot.getNonterminal, Set[HeadGrammarSlot]())
        reachabilityGraph.put(nonterminalGrammarSlot.getNonterminal, set2)
        changed = addAll(set, set2) || changed
        if (isNullable(nonterminalGrammarSlot.getNonterminal)) {
    ...
    def removeUnusedNonterminals(nonterminal: Nonterminal): GrammarBuilder = {
      ...
      nonterminals.filter(referedNonterminals.contains(_))
      this
    }
    ...
    private def removeUnusedNewNonterminals() {
      ...
      for (list <- newNonterminalsMap.values) {
        list.filter(reachableNonterminals.contains(_))
      }
    }
    ...
    private def test(slot: BodyGrammarSlot,
      ...
      if (node.size == 1) {
        val currentSlot = getBodyGrammarSlot(node.edges(0).getLabel, symbolIndex, slot, headGrammarSlot)
        test(currentSlot, node.edges(0).getDestination, symbolIndex + 1, headGrammarSlot)
        return
      }
