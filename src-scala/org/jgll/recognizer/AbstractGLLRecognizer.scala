...
trait AbstractGLLRecognizerTrait {
  self: GrammarTrait
  ...
  object AbstractGLLRecognizer {
    val log = LoggerWrapper.getLogger(classOf[AbstractGLLRecognizer])
  ...
  abstract class AbstractGLLRecognizer extends GLLRecognizer {
    ...
    protected var descriptorStack: collection.mutable.Queue[Descriptor] = _
    ...
    override def recognize(input: Input, grammar: Grammar, nonterminalName: String): Boolean = {
      ...
      L0.recognize(this, input, startSymbol)
    ...
    protected def init(grammar: Grammar,
      ...
      if (descriptorStack == null) {
        descriptorStack = collection.mutable.Queue()
      } else {
    ...
    override def nextDescriptor(): Descriptor = descriptorStack.dequeue
